...
const { renderToFile, renderToStream, Document, Page, StyleSheet, Text, View, Font } = require('@react-pdf/renderer');
  ...
  <Document>
    <Page wrap size="A4" style={styles.page}>
      <View style={styles.header}>
        <Text style={styles.header.name}>{toTitleCase(props?.resume?.firstName)} {toTitleCase(props?.resume?.lastName)}</Text>
      ...
      <View style={styles.body}>
        <View style={styles.body.main}>
            ...
            {props.resume.experiences.map((job) => (
              <View style={styles.body.main.employment.job}>
                <View style={styles.body.main.employment.job.role}>
                  <Text wrap style={styles.body.main.employment.job.title}>{toTitleCase(job.employmentTitle)}</Text>
                  <Text style={styles.body.main.employment.job.date}>{job.fromDate?format(new Date(job.fromDate), 'MMM yy'):''} - {job.thruDate?format(new Date(job.thruDate), 'MMM yy'):'Present'}</Text>
                </View>
                <View>
                  <Text style={styles.body.main.employment.job.location}>{toTitleCase(job.employer.name)}, {job.city? toTitleCase(job.city) : job.state? toTitleCase(job.state): ''}{(job.city || job.state) && job.country?', ':''}{job.country?toTitleCase(job.country) : ''}</Text>
                </View>
          ...
          {props.resume.educations && props.resume.educations.length>0 &&
              ...
              {props.resume.educations.map((edu) => (
                <View wrap style={styles.body.main.education.school}>
                  <View style={styles.body.main.education.school.study}>
                    <Text style={styles.body.main.education.school.degree}>{toTitleCase(edu.degree?edu.degree:'')}{edu.fieldOfStudy?.name? ' of ' + toTitleCase(edu.fieldOfStudy.name):''}{edu.institute?.name?', ' + toTitleCase(edu.institute.name):''}</Text>
                    <Text style={styles.body.main.education.school.date}>{edu.fromDate?format(new Date(edu.fromDate), 'MMM yy'):''} - {edu.thruDate?format(new Date(edu.thruDate), 'MMM yy'):''}</Text>
                  </View>
                  <View>
                    <Text style={styles.body.main.education.school.location}>{edu.institute.primaryAddress.city?toTitleCase(edu.institute.primaryAddress.city):''}{(edu.institute.primaryAddress.city || edu.institute.primaryAddress.state) && edu.institute.primaryAddress.country?', ':''}{edu.institute.primaryAddress.country?toTitleCase(edu.institute.primaryAddress.country):''}</Text>
                  </View>
                </View>
              ))}
            </View>
          }
          {props.resume.skills && props.resume.skills.length>0 &&
            <View wrap>
              <View style={styles.body.sectionHeader}>
          ...
          {props.resume.languages && props.resume.languages.length>0 &&
            <View>
              <View style={styles.body.sectionHeader}>
...
const generateResume = async (data) => {
  // console.log(data)
  renderToFile(<Classic resume={data} />, `${__dirname}/${data.firstName.toLowerCase()}_${data.lastName.toLowerCase()}.pdf`);
}

const resumeToStream = async (data) => {
  const stream = await renderToStream(<Classic resume={data} />);
  return stream;
}
