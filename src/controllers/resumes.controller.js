...
const uploadResume = catchAsync(async (req, res) => {
  ...
  try {
      ...
      if(files.file) {
        ...
        result = await sovrenService.uploadResume(cv.path, 'resume', currentUserId);
        console.log(result)
        //
...
const parseResume = catchAsync(async (req, res) => {
  ...
  try {
    ...
    if(files.file) {
      ...
      const resume = new ResumeParser(cv.path);
      await resume.parseToJSON()
        .then(data => {
          console.log('number', data.parts.phone)
          result = { ...data, skills: data?.parts?.skills?.split(',') };
        })
        .catch(error => {
          console.error(error);
        });
