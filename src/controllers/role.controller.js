...
const getAllPrivileges = catchAsync(async (req, res) => {
  ...
  res.json(_.chain(result)
    .map((privilege) => {
      return {
        ...privilege,
        description: privilege.description[locale]
      };
    })
    .groupBy("type")
...
async function insertPrivilegeInDB(){
  //const data = JSON.stringify(result, null, 2);
  const filePath = path.join(__dirname, '../../data/privilege.json');
  const jsonData = fs.readFileSync(filePath);
  const privileges = JSON.parse(jsonData);
