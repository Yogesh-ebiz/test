...
async function add(emailCampaign) {
  ...
  emailCampaign = await emailCampaignSchema.validateAsync(emailCampaign, { abortEarly: false });
...
async function search(jobId, filter, sort) {
  ...
  aList.push({ $match: {job: jobId, status: {$in: filter.status} } });
  // aList.push(
  //   {
  //     $lookup: {
...
module.exports = {
  ...
  findByEmailAddressAndJobId,
  findByJobId:findByJobId,
  findCampaignsByJobId,
  findByToken,
  search
}
