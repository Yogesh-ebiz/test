...
async function addQuestion(companyId, question) {
  ...
  if(!companyId || !question){
    throw new Error('Company ID and question are required');;
  }
  try {
    ...
    const newQuestion = await new UserQuestion(value).save();

    return newQuestion;
  } catch (e) {
    throw new Error(`Error saving question: ${e.message}`);
  }
}
