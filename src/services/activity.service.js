...
async function add(activity) {
  ...
  await activitySchema.validate(activity, {abortEarly: false});
...
async function findByJobId(companyId, jobId, sort) {
  ...
  let aList = [];
  let aLookup = [];
  let aMatch = { $match: {'$or': [{'meta.job': jobId}, {subject: jobId} ]} };
  let aSort = { $sort: {createdAt: direction} };
